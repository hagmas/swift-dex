...
/// is a change to that rectangle: `zoom` resizes it around an element, `scroll`
/// moves it without changing its size, and `reset` returns it to the slide's
/// home rectangle.
///
/// Because the operations share one rectangle, they compose: a `scroll` after a
/// `zoom` pans at the zoom level the `zoom` established, rather than pulling
/// back out.
///
/// ```swift
...
///     Camera(.scroll(to: .detail))   // stays at 0.5, moves across
///     Camera(.reset)
/// }
/// ```
