...
public enum Placement: Hashable, Sendable {
    ...
    /// A waypoint, and the direction it sends a line away along.
    ///
    /// A waypoint is a node — lines name it and it takes a position — but the
    /// direction it sends them is a fact about the arrangement, so it is
    /// recorded here rather than guessed at from where the waypoint landed.
    case waypoint(NodeID, Axis)
...
public extension Placement {
    ...
    var nodeIDs: [NodeID] {
        switch self {
        case .node(let id), .waypoint(let id, _):
            [id]
        case .gap:
            []
...
extension Placement {
    ...
    /// The direction each waypoint sends its lines away along.
    static func waypointAxes(in placements: [Placement]) -> [NodeID: Axis] {
        var axes: [NodeID: Axis] = [:]

        func walk(_ placements: [Placement]) {
            for placement in placements {
                switch placement {
                case .waypoint(let id, let axis):
                    axes[id] = axis
                case .group(_, let children):
                    walk(children)
                case .node, .gap:
